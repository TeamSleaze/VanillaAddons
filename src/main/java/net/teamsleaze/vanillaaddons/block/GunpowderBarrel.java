    ...
    public void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        if (!world.isClient) {
            ...
            if (projectile.isOnFire() && projectile.canModifyAt(world, blockPos)) {
                GunpowderBarrel.explode(world, blockPos);
...
public class GunpowderBlock extends Block {
    ...
    public GunpowderBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        ...
        if (world.isReceivingRedstonePower(pos)) {
            GunpowderBlock.primeGunpowderBlock(world, pos);
            world.removeBlock(pos, false);
        }
    }
    ...
    public void onDestroyedByExplosion(World world, BlockPos pos, Explosion explosion) {
        ...
        gunpowderBlockEntity.setFuse(5); // After nearby explosion
        world.spawnEntity(gunpowderBlockEntity);
    }

    public static void primeGunpowderBlock(World world, BlockPos pos) {
        GunpowderBlock.primeGunpowderBlock(world, pos, null);
    }

    private static void primeGunpowderBlock(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (world.isClient) {
            return;
        }
        ...
        world.spawnEntity(gunpowderBlockEntity);
        world.playSound(null, gunpowderBlockEntity.getX(), gunpowderBlockEntity.getY(), gunpowderBlockEntity.getZ(), SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.BLOCKS, 1.0f, 1.0f);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player2, Hand hand, BlockHitResult hit) {
        ItemStack itemStack = player2.getStackInHand(hand);
        if (itemStack.isOf(Items.FLINT_AND_STEEL) || itemStack.isOf(Items.FIRE_CHARGE)) {
            GunpowderBlock.primeGunpowderBlock(world, pos, player2);
            world.setBlockState(pos, Blocks.AIR.getDefaultState(), Block.NOTIFY_ALL | Block.REDRAW_ON_MAIN_THREAD);
            Item item = itemStack.getItem();
            if (!player2.isCreative()) {
                if (itemStack.isOf(Items.FLINT_AND_STEEL)) {
    ...
    public void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        if (!world.isClient) {
            ...
            Entity entity = projectile.getOwner();
            if (projectile.isOnFire() && projectile.canModifyAt(world, blockPos)) {
                GunpowderBlock.primeGunpowderBlock(world, blockPos, entity instanceof LivingEntity ? (LivingEntity)entity : null);
                world.removeBlock(blockPos, false);
            }
        }
    }
